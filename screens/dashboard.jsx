...
function DashboardScreen() {
  ...
  return (
    <div>
      ...
      {/* ALERTS BANNER */}
      <Card alert className="mb-16">
        <div className="card__head" style={{borderColor:'var(--brick)', background:'rgba(148,57,42,0.04)'}}>
          <h3 style={{color:'var(--brick)'}}>⚠  Needs attention today</h3>
          <span className="meta">As of {fmtDate(todayIso, {full:true})}</span>
        </div>
        <div className="card__body" style={{padding:0}}>
          <AlertRow
            tone="brick" count={vacateDue.length}
            title={`${vacateDue.length} vacate notice${vacateDue.length===1?'':'s'} due — past day 11`}
            sub={vacateDue.length ? vacateDue.map(r => getTenant(r.tenantId)?.name).filter(Boolean).join(' · ') : 'No tenants currently past vacate threshold.'}
            actionLabel="Generate notices"
            onAction={() => nav('/notice')}
            hidden={vacateDue.length === 0}
          />
          <AlertRow
            tone="ochre" count={outstanding.length}
            title={`${outstanding.length} tenant${outstanding.length===1?'':'s'} with rent outstanding for ${fmtMonthLong(month)}`}
            sub={`${fmtMoney(totalOwed)} still owed across ${outstanding.length} unit${outstanding.length===1?'':'s'}`}
            actionLabel="Open rent roll"
            onAction={() => nav('/rent')}
            hidden={outstanding.length === 0}
          />
          <AlertRow
            tone={offersAwaiting.some(x => x.days >= 3) ? 'brick' : 'ochre'} count={offersAwaiting.length}
            title={`${offersAwaiting.length} offer${offersAwaiting.length===1?'':'s'} awaiting your response`}
            sub={offersAwaiting.slice(0,3).map(x => `${x.property.address} — ${x.offer.buyer || 'buyer'} (${fmtMoney(offerNetToSeller(x.offer))} net · ${x.days}d)`).join(' · ')}
            actionLabel="Review offers"
            onAction={() => nav('/property/' + offersAwaiting[0].property.id + '/offers')}
            hidden={offersAwaiting.length === 0}
          />
          <AlertRow
            tone="ochre" count={untagged.length}
            title={`${untagged.length} transactions need triage`}
            sub="From bank imports — auto-suggest will pre-fill category & property"
            actionLabel="Triage"
            onAction={() => nav('/transactions')}
          />
          <AlertRow
            tone={leaseRenewals.some(x => x.days <= 30) ? 'brick' : 'ochre'} count={leaseRenewals.length}
            title={leaseRenewals.length === 0 ? 'Lease renewals — nothing imminent' : `${leaseRenewals.length} lease${leaseRenewals.length===1?'':'s'} ending within 90 days`}
            sub={leaseRenewals.slice(0,3).map(x => `${x.tenant.name} (${x.days}d · ${fmtDate(x.tenant.leaseEnd)})`).join(' · ')}
            actionLabel="Review"
            onAction={() => nav('/rent')}
            hidden={leaseRenewals.length === 0}
          />
          <AlertRow
            tone={loanMaturities.some(p => daysBetween(todayIso, p.loanDetail.maturityDate) <= 30) ? 'brick' : 'ochre'} count={loanMaturities.length}
            title={`${loanMaturities.length} loan${loanMaturities.length===1?'':'s'} maturing within 90 days`}
            sub={loanMaturities.slice(0,3).map(p => `${p.address} (${fmtDate(p.loanDetail.maturityDate)} · ${p.loanDetail.lender})`).join(' · ')}
            actionLabel="Plan refi"
            onAction={() => nav('/refi')}
            hidden={loanMaturities.length === 0}
          />
          <AlertRow
            tone="ochre" count={insuranceRenewals.length}
            title={insuranceRenewals.length ? `${insuranceRenewals.length} insurance renewal${insuranceRenewals.length===1?'':'s'} within 30 days` : 'Insurance renewals — all clear'}
            sub={insuranceRenewals.slice(0,3).map(p => p.address + ' (' + fmtDate(p.insurance.renewalDate) + ')').join(' · ')}
            actionLabel="Review"
            onAction={() => nav('/properties')}
            hidden={insuranceRenewals.length === 0}
          />
          <AlertRow
            tone="ochre" count={taxesDue.length}
            title={`${taxesDue.length} property tax${taxesDue.length===1?'':'es'} due within 30 days`}
            sub={taxesDue.slice(0,3).map(p => p.address + ' (' + fmtMoney(p.taxes.annualAmount) + ' by ' + fmtDate(p.taxes.dueDate) + ')').join(' · ')}
            actionLabel="Review"
            onAction={() => nav('/properties')}
            hidden={taxesDue.length === 0}
          />
          <AlertRow
          ...
          />
          <AlertRow
            tone="blue" count={staleProps.length}
            title={staleProps.length === 0 ? 'No stale deals' : `${staleProps.length} deals in C-Rehab >60 days`}
            sub={staleProps.slice(0,3).map(p => p.address).join(' · ')}
            actionLabel="View pipeline"
            onAction={() => nav('/pipeline')}
            hidden={staleProps.length === 0}
          />
        </div>
      </Card>
