...
const PROP_COLUMNS = [
  ...
  // Sale side — filled when the property goes under contract with a buyer.
  { key: 'buyerDDDate',    label: 'Buyer DD date',
    render: (p) => <span className="small mono dim">{fmtDate(p.buyerDDDate)}</span>,
    sortValue: (p) => p.buyerDDDate || '' },
  { key: 'saleSigningDate', label: 'Sale signing',
    render: (p) => <span className="small mono dim">{fmtDate(p.saleSigningDate)}{p.saleSigningDate && fmtClock(p.saleSigningTime) ? ' · ' + fmtClock(p.saleSigningTime) : ''}</span>,
    sortValue: (p) => (p.saleSigningDate || '') + (p.saleSigningTime || '') },
  { key: 'expectedCloseDate', label: 'Expected close',
    render: (p) => <span className="small mono dim">{fmtDate(p.expectedCloseDate)}</span>,
    sortValue: (p) => p.expectedCloseDate || '' },
  { key: 'saleAttorney',   label: 'Sale attorney',
    render: (p) => <span className="small dim">{p.saleAttorney || '—'}</span>,
    sortValue: (p) => p.saleAttorney || '' },
