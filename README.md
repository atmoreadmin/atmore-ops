...
function SplitTransactionModal({ tx, onClose }) {
  ...
  function splitEvenly() {
    ...
    const per = Math.trunc(cents / splits.length);
    // Last row absorbs the rounding remainder so the split always balances.
    setSplits(splits.map((s, i) => {
      const c = i === splits.length - 1 ? cents - per * (splits.length - 1) : per;
      return { ...s, amount: c / 100 };
    }));
  }
    ...
    <Modal title={`Split transaction · ${fmtMoney(tx.amount, {sign: true})}`} onClose={onClose}>
      <div className="col gap-14">
        ...
        <div className="row gap-8 mt-8">
          ...
          <div className="grow"/>
          {(!isBalanced || splits.filter(s => s.project).length < 2) && (
            <span className="tiny dim" style={{alignSelf: 'center'}}>
              {!isBalanced ? `Allocate the remaining ${fmtMoney(remaining)} to enable saving` : 'Pick a property/bucket on at least 2 rows'}
            </span>
          )}
